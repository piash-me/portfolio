  ...
  return (
    <div className="glass rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-fg font-medium">Embedded App</h3>
        <span className="mono-font text-[10px] px-2 py-0.5 rounded-full border border-hairline text-fg-muted ml-auto">iframe</span>
      </div>
      ...
      {embedUrl ? (
        <iframe src={embedUrl} className="w-full h-96 rounded-xl border border-hairline" title="Embedded tool" />
      ) : (
        <div className="w-full h-56 rounded-xl border border-dashed border-hairline flex flex-col items-center justify-center text-center px-6">
          <p className="text-fg-muted text-sm mb-3">No embed URL set yet — this is where your tool will render.</p>
          <span className="mono-font text-[11px] text-fg-muted">e.g. https://your-tool.streamlit.app</span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="glass rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-fg font-medium">API-backed Tool</h3>
        <span className="mono-font text-[10px] px-2 py-0.5 rounded-full border border-hairline text-fg-muted ml-auto">/api/tools</span>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-5">
        <div>
          <label className="text-xs text-fg-muted block mb-1.5">Daily deliveries</label>
          <input type="number" value={rows} onChange={(e) => setRows(Number(e.target.value))}
            className="w-full bg-glass border border-hairline rounded-lg px-3 py-2 text-sm text-fg outline-none focus:border-violet/50" />
        </div>
        <div>
          <label className="text-xs text-fg-muted block mb-1.5">Current late rate (%)</label>
          <input type="number" value={lateRate} onChange={(e) => setLateRate(Number(e.target.value))}
            className="w-full bg-glass border border-hairline rounded-lg px-3 py-2 text-sm text-fg outline-none focus:border-violet/50" />
        </div>
      </div>
      <button onClick={runTool} disabled={loading}
        className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-btn-primary text-btn-primary-fg text-sm font-medium hover:opacity-90 transition-colors disabled:opacity-50">
        <Play size={14} /> {loading ? 'Calculating…' : 'Run prediction'}
      </button>
      {result && (
        <div className="mt-5 grid grid-cols-2 gap-4">
          <div className="rounded-xl bg-glass p-4">
            <p className="mono-font text-2xl text-fg">{result.projectedBreaches}</p>
            <p className="text-xs text-fg-muted mt-1">Projected breaches / day</p>
          </div>
          <div className="rounded-xl bg-glass p-4">
            ...
            <p className="text-xs text-fg-muted mt-1">Risk level</p>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <main className="min-h-screen pt-24 px-6 pb-20">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="display-font text-3xl sm:text-4xl text-fg mb-3">Two patterns, one project schema.</h1>
        <p className="text-fg-muted mb-10 max-w-xl">
