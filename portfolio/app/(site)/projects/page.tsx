  ...
  return (
    <main className="min-h-screen pt-24 px-6 pb-24">
      <div className="max-w-5xl mx-auto">
        ...
        <h1 className="display-font text-3xl sm:text-4xl text-fg mb-10">Every project, one place.</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-10">
          <div className="flex items-center gap-2 glass rounded-full px-4 py-2.5 flex-1">
            ...
            <input value={query} onChange={(e) => { setQuery(e.target.value); setVisibleCount(PAGE_SIZE); }}
              placeholder="Search projects" className="bg-transparent outline-none text-sm text-fg placeholder-neutral-500 w-full" />
          </div>
        ...
        <p className="text-fg-faint text-xs mb-6">{filtered.length} project{filtered.length !== 1 ? 's' : ''}</p>
        ...
        {filtered.length === 0 && <p className="text-fg-muted text-sm text-center py-16">No projects match that search yet.</p>}
